...
public:
  ...
  void init() {
    ...
    // 播放开机音效 (第二首音频, 索引为 1)
    if (flashOk_) {
      if (audioPlayer_.playTrack(1)) {
        uart_.log("[MAMBA] Playing power-on audio (track 1)\r\n");
      } else {
        uart_.log("[MAMBA] Power-on audio not found, skipping\r\n");
      }
    }
  }
  ...
  void loop() {
    ...
    uartTransfer_.updateStatus(
        batteryMonitor_.getVoltage(), batteryMonitor_.getBatteryPercent(),
   ...
   */
  void onTimerUpdate() { audioPlayer_.onTimerUpdate(); }
...
private:
  ...
  hdl::WS2812Driver ws2812_{WS2812_DIN_GPIO_Port,
                            WS2812_DIN_Pin}; ///< WS2812 驱动
  ...
  fml::UartTransfer uartTransfer_{uart_, flash_,
                                  batteryMonitor_}; ///< UART 传输
