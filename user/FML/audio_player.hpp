 ...
 *
 *          音频规格：
 *          - 采样率：32kHz (由 TIM1 RCR 分频决定: 256kHz PWM / RCR=7 = 32kHz)
 ...
 *
 *          音量保护：
 *          - PWM 最大占空比限制为 95%，防止扬声器(额定1W,最大1.5W)过载
 *          - volume_scale_ 范围 0~95，默认 80
 */
...
public:
  ...
  /// 音量缩放最大值 (95% 占空比限制，保护扬声器)
  /// 最大 CCR = sample * 125 / 128 * volume / 100
  /// 当 volume=95 时，最大 CCR = 255 * 125 / 128 * 80 / 100 ≈ 199 (< 250)
  static constexpr uint8_t MAX_VOLUME = 95;
  ...
  AudioPlayer(hdl::W25Q64Driver &flash, hdl::AmpControl &amp, hal::Pwm &pwm)
      : flash_(flash), amp_(amp), pwm_(pwm), playing_(false), looping_(false),
        volume_(DEFAULT_VOLUME), currentAddr_(0), remainBytes_(0), bufIndex_(0),
        bufPos_(0), bufReady_{false, false}, toneMode_(false), tonePhase_(0),
        tonePeriodSamples_(0), toneSamplesLeft_(0), needFill_(false) {
   ...
   *          正弦波查表法生成：
   *          - 采样率 = 32000 Hz
   *          - 每个周期的采样点数 = 32000 / frequency
   *          - 例如 440Hz: 32000/440 ≈ 72.72 个采样点
   */
  void playTone(uint16_t frequency, uint32_t durationMs = 0) {
    ...
    toneMode_ = true;
    tonePhase_ = 0;
    // 每个周期的采样点数 (32kHz / frequency)
    tonePeriodSamples_ = 32000 / frequency;
    if (tonePeriodSamples_ == 0)
      tonePeriodSamples_ = 1;

    // 持续时间转换为采样点数
    if (durationMs > 0) {
      toneSamplesLeft_ =
          static_cast<uint32_t>(32) * durationMs; // 32 samples/ms
    } else {
  ...
  /**
   * @brief 暂停播放
   * @details 停止硬件输出但保持当前播放位置，可通过 resume() 继续。
   */
  void pause() {
    if (!playing_ || isPaused_)
      return;
    isPaused_ = true;
    pwm_.stopWithIT();
    amp_.disable();
  ...
  /**
   * @brief TIM Update 中断回调 — 每 31.25μs (32kHz) 调用一次
...
private:
  ...
  volatile bool isPaused_; ///< 是否处于暂停状态
  bool looping_;          ///< 是否循环播放
  uint8_t volume_;        ///< 音量 (0~95)
  ...
  // 需要在主循环中填充缓冲区标志 (必须在 toneSamplesLeft_
  // 之后声明以匹配初始化顺序)
  ...
  uint8_t generateToneSample() {
    ...
    if (phase < quarter) {
      // 第一象限：128 → 255
      sample = 128 + static_cast<uint8_t>(static_cast<uint32_t>(127) * phase /
                                          quarter);
    } else if (phase < 2 * quarter) {
      // 第二象限：255 → 128
      sample = 255 - static_cast<uint8_t>(static_cast<uint32_t>(127) *
                                          (phase - quarter) / quarter);
    } else if (phase < 3 * quarter) {
      // 第三象限：128 → 1
      sample = 128 - static_cast<uint8_t>(static_cast<uint32_t>(127) *
                                          (phase - 2 * quarter) / quarter);
    } else {
      // 第四象限：1 → 128
      sample = 1 + static_cast<uint8_t>(static_cast<uint32_t>(127) *
                                        (phase - 3 * quarter) / quarter);
    }
